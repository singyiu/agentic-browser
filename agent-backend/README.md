...
# Per-profile classification prompt (age-aware); GET ?profile=<kid|global>, POST {profile,prompt,age?}.
curl -s -H "X-Guardian-Parent-Pin: $PIN" "http://127.0.0.1:2947/review/prompt?profile=global"
curl -s -X POST -H "X-Guardian-Parent-Pin: $PIN" -H 'Content-Type: application/json' \
  -d '{"profile":"alice","prompt":"Allow educational coding sites.","age":12}' \
  http://127.0.0.1:2947/review/prompt
curl -s -X POST -H "X-Guardian-Parent-Pin: $PIN" -H 'Content-Type: application/json' \
